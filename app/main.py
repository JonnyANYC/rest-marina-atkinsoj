...
class BoatListHandler(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        # FIXME: Add error handling for missing fields. Need to test for property existence in request_data.
...
class BoatHandler(webapp2.RequestHandler):
    ...
    def patch(self, boat_id):
        ...
        if "at_sea" in request_data and request_data["at_sea"]:
            if request_data["at_sea"] == "false":

                # FIXME: This is all old and prolly untested and not needed here any more.
                self.response.headers['Content-Type'] = 'application/json; charset=utf-8'
                self.response.write(response_message_json("FAILURE", "Use PATCH /slips/:id to dock a boat in a slip."))
                return

            if request_data["at_sea"] == "true" and not boat.at_sea:

                slip = Slip.find_slip_by_boat_id(boat_id)
                self.response.write(response_message_json("SUCCESS sorta", "Found it! slip # " + slip.number + " has boat # " + slip.current_boat))
                slip.current_boat = None
                slip.arrival_date = None
                slip.put()
                boat.at_sea = True
...
class BoatDockHandler(webapp2.RequestHandler):
    ...
    def patch(self, boat_id):
        ...
        if "slip_id" not in request_data:
            send_error(self.response, 400, response_message_json("FAILURE", "slip_id property missing. " 
                       "Provide empty slip_id to set a boat at sea."))
            return

        # send_success(self.response, "slip id: " + request_data["slip_id"])
        # send_success(self.response, "slip is null? " + str(request_data["slip_id"] is None))
        if request_data["slip_id"] is None:
            ...
            current_slip = Slip.find_slip_by_boat_id(boat_id)
...
class SlipListHandler(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        # FIXME: Set the arrival_date to now(), but in the required string format
